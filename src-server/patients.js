...
const { setUpGetStream, setUpDelStream, setUpCreateStream } = require(`./common`);
...
setUpCreateStream(newPatientSubject, `patients`, () => {
  const query = `insert into patient (id, person_id) values (nextval('patient_id_seq'), $1)`;

  return (data, db) => db.query(query, [data[0].id]);
});
